...
There is no linter config or build step. The Elermore Vale GLM→OpenDSS translation has a pytest suite (`python -m pytest`, ~4 s: unit tests, source-vs-circuit invariants, physics sanity power flows — see `MODEL_VERIFICATION.md`; census/golden constants in `tests/` are measured ground truth), plus a GridLAB-D cross-validation harness under `validation/` (voltages agree to ~1 % mean). Two deliberate modelling decisions documented in `MODEL_VERIFICATION.md` "Known defects": the 40 Redflow batteries are built as dispatchable `Generator` elements, not `Storage` (2+ active Storage elements make DSS C-API 0.14.5 collapse to a dead circuit that still reports Converged=True), and bare GLM line lengths are FEET per GridLAB-D semantics (`glm_length_m`) — Elermore Vale network results generated before 2026-08-13 used 3.28× inflated 11 kV impedances and should be regenerated. Everything else is verified by running the scripts and inspecting logged metrics (e.g. mean annual savings ~$430 FiT / ~$190 net) and generated figures.
