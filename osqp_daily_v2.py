...
def run_all(day_arrays, mode, e_max=E_MAX_DEFAULT):
    ...
    with Pool(processes=n_proc) as pool:
        for cust, total in pool.imap_unordered(_worker, jobs, chunksize=1):
            logger.info("Customer %s: $%.2f/yr", cust, total)
            customers.append(cust)
            savings.append(total)
    order = np.argsort(customers)
    return np.array(customers)[order], np.array(savings)[order]
...
def main():
    ...
    customers_fit, savings_fit = run_all(day_arrays, mode="fit")
    customers_net, savings_net = run_all(day_arrays, mode="net")
