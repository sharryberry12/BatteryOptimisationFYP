...
| Path | Purpose |
|------|---------|
| [osqp_daily_openDSS.py](osqp_daily_openDSS.py) | QP battery scheduler — implements [R15] Sections 4-5 with a persistent OSQP workspace. Produces the half-hourly dispatch profiles consumed by every network model. |
...
| [profiles/](profiles/) | Output of `osqp_daily_openDSS.py`: half-hourly load/PV/battery/grid time series in long-format CSV. `fit_profiles.csv` = topology 1 (gross FiT); `net_profiles.csv` = topology 2 (net metering). |
