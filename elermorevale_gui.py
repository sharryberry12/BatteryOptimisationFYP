#!/usr/bin/env python3
"""
elermorevale_gui.py — Improved dual-view power flow dashboard.
...
This is a refactor with the following additions:
...
Usage:
    ...
    python elermorevale_gui.py --serve --port 8765
    ...
    for lbl, ub in [("baseline", True), ("qp", False)]:
        ...
        try:
            ...
            if "485" in msg:
                ...
                logger.warning("Solver max iterations for %s: %s", lbl, msg)
            else:
                scen_health["converged"] = False
                logger.error("Solver failed for %s: %s", lbl, msg)
                # Critical failure — don't continue with invalid circuit data
                res[lbl] = {
                    "voltages": {},
                    "tx_p": [],
                    "tx_q": [],
